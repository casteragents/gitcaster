...
async function main() {
  ...
  const evidence = {
    ...
    summary: {
      ...
      publicDocsFound: fs.existsSync(path.join(repoRoot, "apps/web/public/gitcaster-mcp-source.md")),
      localToolPlanFixtureFound: fs.existsSync(path.join(repoRoot, "examples/mcp/local-tool-plan.example.json")),
    ...
    releaseQuality: {
      ...
      productionBlockers: [
        "MCP source is public-alpha only; no public MCP gateway is claimed.",
