...
if (!packageOnly) {
  if (!exists("launch/evidence/pr-12-web-status-proof-ui.json")) {
    ...
    if (evidence.summary?.casterDidIdentityPageCreated !== true) blockers.push("casterDidIdentityPageCreated must be true");
    if (evidence.summary?.casterDidIdentityEvidenceFound !== true) blockers.push("casterDidIdentityEvidenceFound must be true");
    if (evidence.summary?.casterDidIdentityPublicSmokeFound !== true) blockers.push("casterDidIdentityPublicSmokeFound must be true");
